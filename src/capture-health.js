...
function battleReplyCount(dom) {
  const battle = dom && (dom.battle || (dom.responses || dom.lanes ? dom : null));
  if (!battle) return 0;
  let n = 0;
...
function assistantContentCount(dom) {
  const msgs = (dom && dom.messages) || [];
  let n = 0;
  msgs.forEach(function (m) {
    if (!m || m.role !== "assistant") return;
    const blocks = m.content || [];
    const has = blocks.some(function (b) {
      if (!b) return false;
...
function looksLikeTermsOrEmptyHome(dom) {
  ...
  const battleN = battleReplyCount(dom);
  const asst = assistantContentCount(dom);
  const msgs = (dom.messages || []).length;
...
function evalCaptureCount(ic) {
  ic = ic || {};
  let n = ic.evaluationStreamCount || 0;
...
function agentStreamCount(ic) {
  ic = ic || {};
  let realtime = ic.agentRealtimeOutCount || 0;
  let chat = ic.agentChatCount || 0;
...
function isAgentContext(input, battleN) {
  ...
  const url = String((input.dom && input.dom.url) || (input.interceptor && input.interceptor.url) || "");
...
AE.captureHealth = function (input) {
  input = input || {};
  const ic = input.interceptor || {};
  const dom = input.dom || {};
  const warnings = [];
  let critical = false;

  const urlOnly = input.urlOnlyFiles || [];
  if (urlOnly.length) {
    const fileWarn = AE.urlOnlyFilesWarning(urlOnly);
...
AE.mergeHealthWarnings = function (existing, health) {
  const kept = (existing || []).filter(function (w) { return !AE.isCaptureHealthWarning(w); });
  const fresh = (health && health.warnings) || [];
...
AE.scoreCompleteness = function (payload) {
  ...
  const reasons = [];
  const subtype = (AE.firstBattleSubtype && AE.firstBattleSubtype(payload)) || (battles.length ? "text" : "agent");
  ...
  let expected = 0, withBytes = 0, urlOnly = 0;
  ...
  const media = subtype === "image" || subtype === "video";
  let prose = false;
  battles.forEach(function (b) {
    (b.contestants || []).forEach(function (c) {
  ...
  const emptyShell = media && withBytes === 0 && !prose;
  ...
  const warnings = (payload.meta && payload.meta.warnings) || [];
  ...
  if (warnings.some(function (w) { return /Export reconstructed from DOM|DOM backfilled|capture may have started|Backfilled from Arena history API|truncated|persistence failed/i.test(w); })) {
    if (status === "green") status = "amber";
    reasons.push("capture may be incomplete");
  }
  if (Object.keys(meta.evaluation_streams || {}).length || battles.some(function (b) { return b.stream_error; })) {
    if (status === "green") status = "amber";
    reasons.push("evaluation stream incomplete or unrecognized");
  }
  if (meta.transcript && meta.transcript.pagination && meta.transcript.pagination.hasMore) {
    if (status === "green") status = "amber";
    reasons.push("earlier transcript pages not captured");
  }
  const attempts = meta.request_attempts || [];
  const lastAttempt = attempts[attempts.length - 1];
  if (lastAttempt && /error|rejected|aborted/.test(lastAttempt.outcome || "")) {
    if (status === "green") status = "amber";
    reasons.push("latest request: " + lastAttempt.outcome);
  }
...
AE.shouldSkipEmptyArchive = function (payload) {
  if (payload && payload.meta && (payload.meta.request_attempts || []).length) return false;
  const score = AE.scoreCompleteness(payload);
...
AE.applyCompletenessMeta = function (payload, extraWarnings) {
  ...
  const extra = extraWarnings || [];
  const reasons = (score.reasons || []).map(function (r) { return "Completeness: " + r; });
  const merged = (payload.meta.warnings || []).slice();
