...
AE.openConversationFolder = async function (request) {
  ...
  const pageKey = conversationKeyFromUrl(tab.url);
  ...
  const key = canonicalSessionKey(pageKey);
  let index = await AE.archiveIndexLoad();
  let entry = index[key] || index[pageKey];
  if (!entry) {
    const sync = await runTurnSync("manual", key, tab.id);
  ...
  const latest = destinations[0] || "downloads:" + AE.ARCHIVE_DIR;
  ...
  const result = await AE.writeArchiveFile(AE.ARCHIVE_DIR + "/" + entry.rel + "/_open-folder.txt",
