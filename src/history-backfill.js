...
(function () {
  ...
  function modelOf(message, evaluation, lane) {
    var m = message || {};
    var name = m.modelName || m.model || m.model_name || m.publicName || m.modelId || m.model_id;
    if (name && !AE.isPlaceholderModel(name)) return String(name);
    var ev = evaluation || {};
  ...
  AE.historyAgentToPayload = function (agent, meta) {
    ...
    var messages = (Array.isArray(agent.messages) ? agent.messages : []).map(function (m, i) {
      return {
        ...
        content: agentPartsToBlocks(m.parts)
      };
    });
