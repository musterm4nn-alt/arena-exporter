...
(function () {
   ...
   */
  AE.parseBattleStream = function (text) {
    ...
    var rowRe = /([ab])([0-9a-e]):/y;
    while (i < text.length) {
      rowRe.lastIndex = i;
      var m = rowRe.exec(text);
      if (!m) { i++; continue; }
      var lane = m[1], code = m[2];
  ...
  AE.scrubSecrets = function (value) {
    var SECRET = /^(recaptcha|g-recaptcha|captcha|authorization|cookie|set-cookie|api[_-]?key|secret|password|passwd|token)$/i;
    var SECRET_SUB = /recaptcha|g-recaptcha|authorization|api[_-]?key/i;
    function walk(v) {
      if (Array.isArray(v)) return v.map(walk);
      if (!v || typeof v !== "object") return v;
      var out = {};
      Object.keys(v).forEach(function (k) {
        if (SECRET.test(k) || SECRET_SUB.test(k)) return;
        out[k] = walk(v[k]);
      });
      return out;
    }
    if (typeof value === "string") {
      try { return JSON.stringify(walk(JSON.parse(value))); } catch (e) { return value; }
    }
    return walk(value);
  };
