...
(function () {
  ...
  function database() {
    if (!opening) opening = new Promise(function (resolve, reject) {
      const request = indexedDB.open("arena-github-backup", 2);
      request.onupgradeneeded = function () {
        const store = request.result.objectStoreNames.contains("pending")
  ...
  AE.backupStore = {
    list: function (target, limit) {
      return transaction("readonly", function (store, done) {
        const request = target ? store.index("target").getAll(IDBKeyRange.only(target), limit || 10) : store.getAll();
