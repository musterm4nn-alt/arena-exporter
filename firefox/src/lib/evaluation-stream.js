...
AE.parseEvaluationStream = function (text, init) {
  const direct = init && /^(direct|direct-battle)$/.test(init.mode || "");
  ...
  const lane = { text: "", reasoning: "", finished: false, finishReason: null, citations: [], tools: [], toolNames: [], files: [] };
  const tools = {};
  function tool(chunk) {
