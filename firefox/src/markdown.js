...
AE.renderMarkdown = function (payload) {
  ...
  if (latest) {
    ...
    lines.push("- Models: " + (contestants.length && named.length === contestants.length
      ...
      : "pending"));
    if (latest.outcome !== "not_applicable") lines.push("- Outcome (latest): " + (latest.outcome || "pending"));
  }
  lines.push("");
  ...
  if (battles.length) {
    battles.forEach(function (b, i) {
      ...
      if (!selectedMode) lines.push("Vote: " + (b.outcome || "pending"));
      lines.push("");
      (b.contestants || []).forEach(function (c) {
        lines.push("### " + (c.lane || "?"));
        lines.push("");
        lines.push(c.model ? "(" + c.model + ")" : "(anonymous)");
        if (c.model_source === "request_catalog") lines.push("Selection label from the page catalog; underlying model identity is unverified.");
        lines.push("");
        if (c.response) {
